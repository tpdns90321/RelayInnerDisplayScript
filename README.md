...
- Spec 31 now extends the Moonlight path with relay-managed persistent client identity plus pinned-host state, live pair-status checks, session-launched `moonlight pair --pin` assist, `waiting_for_pairing` session flow, and runtime pairing metadata without storing Sunshine web-UI credentials.
- Spec 32 now completes the Moonlight path with live-pair-confirmed `Desktop` launch fallback, live non-Desktop app-list validation, `--quit-after` launch rules, reconnect recovery, `moonlight_app` runtime metadata, and operator-facing ops documentation.
- Spec 40 now formalizes the follow-up Moonlight hardening contract: live pair-confirmed `Desktop` fast-path launch and preserved pair state across non-Desktop app-validation degradation.
...
Before launching either `cage` or `sway`, the kiosk launcher now also applies the shared `[display].drm_compatibility` policy through wlroots DRM env overrides. The default `auto` path sets `WLR_DRM_NO_MODIFIERS=1`, which can avoid some monitor-specific atomic modeset failures; `legacy-drm` also sets `WLR_DRM_NO_ATOMIC=1` as a stronger fallback, and `off` disables both relay-managed overrides.
...
If the appliance enters `degraded` with `backend=moonlight`, verify that Linux `moonlight-qt` version `6.0.0` or newer is installed, `[console.moonlight].host`, `base_port`, `app`, and optional `resolution` point at the intended Sunshine target, and the managed `state_dir` is writable by the relay session user. The relay uses that workspace for Moonlight client identity and pinned host certificates, but pair truth comes from live Sunshine `serverinfo` `PairStatus`; if `app = "Desktop"`, it streams directly only after that live probe reports paired, and if `app` names another Sunshine entry, it matches that app case-insensitively against the live Sunshine app list. If you only moved the target VM or changed its IP, keep the same `state_dir`; the daemon now asks Moonlight to probe the configured host once before it decides that a new pairing PIN is required. `quit_app_after_session = true` is only valid for non-`Desktop` apps, and `moonlight_resolution` in `/run/relayinner-display/daemon.state.json` shows the configured requested override rather than a negotiated stream mode. If the kiosk instead stays in `waiting_for_pairing`, approve the PIN shown by Moonlight's pairing UI or in `/run/relayinner-display/daemon.state.json` on the Sunshine web UI `PIN` page. If `moonlight list <host> --csv` still hangs for a non-`Desktop` app despite the headless helper env, the daemon aborts that helper after `[policy].command_timeout_s` and records the timeout in the degraded reason without discarding the existing confirmed pair state.
...
- `relayinner_display.config` now validates the shared TOML config model from Specs 10, 12, 13, 20, 21, 22, 30, 32, 41, and 50, including the backend-neutral `[console]` namespace, loopback-only VNC settings, Looking Glass preflight options, Moonlight host/workspace config, the optional Moonlight resolution override, the `Desktop` plus `quit_app_after_session` exclusion rule, the `[kiosk].compositor` contract, the `[display].drm_compatibility` policy, and legacy SPICE path compatibility.
...
- `relayinner_display.daemon` now owns the end-to-end appliance state machine, validates required runtime binaries, emits backend-neutral `connect_console` IPC, prepares SPICE, VNC, Looking Glass, or Moonlight console launches, runs Looking Glass shared-memory preflight, prepares the managed Moonlight workspace with `portable.dat`, reuses the relay session's HOME and XDG runtime context for daemon-side Moonlight helper commands, enforces `moonlight-qt >= 6.0.0`, probes Moonlight host reachability, persists Moonlight client identity and pinned host certificates in the managed workspace, confirms pairing from live Sunshine `serverinfo` `PairStatus`, lets Moonlight confirm an existing pairing after configured host/IP changes before it reissues the pair UI, normalizes the managed Moonlight host record to the configured host after that recovery succeeds, launches Moonlight's own pairing UI inside the kiosk session with a relay-generated PIN when the host is unpaired, skips daemon-side app-list validation for live-paired `Desktop` streams while keeping live CSV validation for other Moonlight apps, applies an optional Moonlight `--resolution <width>x<height>` override on each stream launch, re-enters reconnect flow after unexpected Moonlight exits, preserves confirmed pair state across non-pairing degraded paths, exposes `waiting_for_pairing`, `moonlight_app`, `moonlight_resolution`, `kiosk_compositor`, and `display_drm_compatibility` runtime metadata, degrades after repeated local Proxmox failures, captures host power-button intent, and writes the expanded runtime state contract to disk.
...
- `relayinner_display.kiosk_launcher` now provides the relay-managed kiosk launcher that resolves the configured compositor to `cage` or `sway` without a shell wrapper, applies the shared wlroots DRM compatibility env policy before exec, writes a generated minimal `/run/relayinner-display/sway.config` for the managed sway path, and warns while falling back gracefully if an explicit sway output pin is unavailable at startup.
...
- `relayinner_display.bootstrap` renders the sample config, systemd units, logind override, host-detected DRM supplementary groups for the kiosk unit, compositor-specific host package requirements based on the current config, the Spec 15 `StartLimitIntervalSec=120` / `StartLimitBurst=5` restart-loop policy, the Spec 16 install-state record under `/var/lib/relayinner-display/install-state.json`, the Spec 22 Looking Glass sample config hints, the Spec 30, 32, and 41 Moonlight sample config hints, and the Spec 17 uninstall flow that restores `tty1` plus optional display-manager state conservatively.
- `tests/` now cover config parsing, backend-neutral IPC validation including `cwd` and waiting `details`, Proxmox command handling, SPICE, VNC, Looking Glass, and Moonlight launch/reconnect logic, Moonlight pair-assist polling and PIN renewal, live Moonlight pair-state validation, Desktop fast-path versus non-Desktop live Moonlight app-list validation, Moonlight resolution override parsing and launch ordering, daemon DPMS debounce behavior, Moonlight workspace/version validation, runtime-state/backend handling including `kiosk_compositor`, `display_drm_compatibility`, `vnc_endpoint`, `looking_glass_shm_file`, `moonlight_app`, `moonlight_resolution`, and Moonlight pairing metadata, runtime dependency degradation, restart-threshold rendering, install-state persistence, uninstall fallback and purge behavior, session supervision, logind policy parsing, power-button handling, display-power handling, managed sway config generation, compositor-specific package selection, and kiosk launcher plus entrypoint wiring.
...
- [Spec 60: Moonlight Live Pair-Status Validation](./specs/60-moonlight-live-pair-status-validation.md)
...
- Wave 6 is complete for Moonlight pair-truth hardening: Spec 60 now validates live Sunshine `PairStatus` before the relay trusts persisted workspace host data.
...
When `console_backend = "moonlight"`, operators also need Linux `moonlight-qt` version `6.0.0` or newer plus a guest that already runs Sunshine. Under the default `[kiosk].compositor = "auto"` contract, Moonlight now resolves to the managed `sway` kiosk path while the other backends stay on `cage`. The relay prepares the managed Moonlight workspace, keeps Moonlight client identity and pinned-host certificates there across restarts, confirms pairing from live Sunshine `serverinfo` `PairStatus` before it treats the current host as paired, launches live-paired `Desktop` streams directly, validates non-`Desktop` app names from live `moonlight list --csv` output before each launch, optionally injects `--resolution <width>x<height>` from config on each relay-managed launch, launches Moonlight's own pairing UI with a relay-generated PIN when the host is not yet paired, drives approval through the Sunshine web UI `PIN` page, mirrors the configured requested resolution into `moonlight_resolution` runtime diagnostics, writes `/run/relayinner-display/sway.config` as runtime-only managed sway state, and still does not automate Sunshine setup or store Sunshine usernames or passwords.
...
- `docs/` holds operator-facing setup documentation for the host-direct install path and the confirmed TDD-cycle workflow.
